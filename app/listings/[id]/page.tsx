        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            <div className="card bg-brand-50 border-brand-100">
              ...
              <div className="text-xs text-brand-500 mt-2">No bid below this amount will be accepted.</div>
            </div>
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-400">{format(new Date(listing.created_at), 'MMM d, yyyy')}</div>
                  </div>
                </div>
                <div className="flex items-start gap-2">
                  <Clock size={14} className={`flex-shrink-0 mt-0.5 ${listing.status === 'active' ? 'text-amber-400' : 'text-gray-300'}`} />
                  <div>
            ...
            {!isOwner && listing.status === 'active' && (
              <div className="card text-center">
                ...
                <p className="text-xs text-gray-500 mb-3">Submit your commission bid and a cover letter to the homeowner.</p>
                <a href="/realtor" className="btn-primary w-full justify-center text-sm">Place a bid</a>
              </div>
            )}
            ...
            <div className="card">
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={row.label} className={`flex justify-between text-xs py-1.5 px-2 rounded-lg ${(row as any).bold ? 'bg-brand-50 font-medium' : ''}`}>
                    <span className={(row as any).muted ? 'text-gray-400 line-through' : 'text-gray-600'}>{row.label}</span>
                    <span className={(row as any).bold ? 'text-brand-600 font-semibold' : (row as any).green ? 'text-brand-500' : 'text-gray-300'}>
